import 'package:flutter/material.dart';

class AppColors {
  static const primaryColor = Color(0xFF1E88E5);
  static const secondaryColor = Color(0xFF1E88E5);
  static const backgroundColor = Color(0xFFF5F5F5);
  static const textColor = Color(0xFF333333);
  static const Color secondaryOrange = Color(0xFFD24F0C); // Hex: #D24F0C
  static const Color white = Color(0xFFffffff);
  static const Color grey = Color(0xFFD3D3D3);
  static const Color hintColor = Color(0xFFBABABA);
  static const Color lightBlue = Color(0xFFEFF6FE);
  static const Color black = Color(0xFF000000);
  static const Color lightOrange = Color(0xFFFCF3EE);
  static const Color lightGrey = Color(0xFFF5F5F5);
  static const Color editFieldBorderColour = Color(0xFFD1E1F5);
  static const darkBlueTextColor = Color(0xFF0B3B6B);
  static const skyBlueTextColor = Color(0xFF1A5695);
  static const dividerColor = Color(0xFFEBEBEB);



}